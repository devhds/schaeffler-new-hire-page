...
const GalleryItems = ({ items }: { items: GalleryTypes["items"] }) => {
  ...
  const handleDrag = useCallback(
    (_: MouseEvent, { offset: { x: dragOffset } }: PanInfo) => {
      ...
      for (
        ...
        const itemOffsetWidth = galleryItemsRef.current[i]?.offsetWidth || 0;
  ...
  return (
    <>
      ...
      <div
        ...
        className={`overflow-hidden sm:px-6 md:pr-9 lg:pr-8 xl:pr-16 xs:px-6 ul:pr-16`}
      >
        <motion.div
          style={{
            x: animatedX,
          ...
          {items?.map((item, index) => (
            ...
            >
              {typeof item.image === "object" &&
                item.image?.mimeType?.includes("image") &&
                item.image.url && (
                  <Image
                    src={item.image.url}
      ...
      <div className="hidden flex-col sm:flex sm:px-6 sm:pt-2 xs:flex xs:px-6 xs:pt-2">
        <motion.div
          variants={variants}
          key={currentElementIndex}
          initial="disable"
