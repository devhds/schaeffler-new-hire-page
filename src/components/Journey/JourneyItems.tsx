...
import { JourneyTypes } from "@/payload-types";
...
const JourneyItems: React.FC<JourneyTypes["items"][number]> = ({
  copy,
  headline,
  description,
  image,
  ...
  const [dynamicImageValues, setDynamicImageValues] = useState<ImageValues>({
    width: "33.4%",
    padding: "3rem",
    offsetTop: ["22%", "42%"],
    offsetY: "-20%",
  });
  const [dynamicTitleValues, setDynamicTitleValues] = useState<TitleValues>({
    offsetTop: ["22%", "42%"],
  });
  ...
  useEffect(() => {
    if (mediaQuery.xs || mediaQuery.sm) {
      ...
      setImageHeight(imageRef.current?.getBoundingClientRect().height * 0.334);
    } else {
      setDynamicImageValues({
        width: "33.4%",
        padding: "3rem",
  ...
  return (
    ...
    >
      ...
      >
        <RichText data={copy} isJourneyItem />
      </div>
    </GridColumnsLayout>
  );
};
