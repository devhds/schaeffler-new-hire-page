...
import { Home, TabsTypes } from '@/payload-types'

const Tabs: React.FC<TabsTypes> = ({ ...props }) => {
  const { headline, description, tabsList } = props
  const tabRef = useRef<HTMLDivElement>(null)
  const tabBarRef = useRef<HTMLDivElement>(null)
  const [value, setValue] = useState<string>(tabsList[0].value)
  const [currentTabContent, setCurrentTabContent] = useState<Home['contentBlocks']>(
  ...
  useEffect(() => {
    const currentContent = tabsList.filter((tab) => tab.value === value)[0]
    setCurrentTabContent(currentContent.tabsContent ?? [])
  }, [value, tabsList])

  useEffect(() => {
    if (tabBarRef.current) {
      setScrollMarginTop(tabBarRef.current.getBoundingClientRect().height * 2)
    }
  }, [])
    ...
    const handleScroll = () => {
      if (stickyTabRef.current) {
        const headerHeight = getComputedStyle(document.documentElement).getPropertyValue(
          mediaQuery.xs || mediaQuery.sm ? '--headerHeightMobile' : '--headerHeight',
        )
        const headerHeightNum = parseFloat(headerHeight)
        setIsSticky(
          window.scrollY + headerHeightNum + stickyTabRef.current.getBoundingClientRect().height >=
  ...
  return (
    <GridColumnsLayout additionalStyles="sm:py-8 md:py-[72px] lg:py-20 xl:py-20 xs:py-8 ul:py-20">
      {headline && (
        ...
        >
          <Headlines element="h2" color="text-primary-soft-black" text={headline} />
        </div>
      )}
      ...
      {headline && description && (
        ...
        >
          <BodyText size="large" color="text-primary-soft-black" text={description} />
        </div>
      )}
      ...
      >
        <TabContext value={value}>
          ...
          >
            {tabsList.map((tab) => (
              <Tab ref={tabBarRef} key={tab.id} label={tab.label} value={tab.value} />
            ))}
          </TabList>
        </TabContext>
      </div>
