...
interface NavigationProps extends NavigationTypes {
  textContentOnly?: Config["collections"]["pages"]["textContentOnly"];
  goBackButtonText?: Config["collections"]["pages"]["goBackButtonText"];
  specialHrefForTranslation?: Record<string, string>;
}

const Navigation = ({ ...props }: NavigationProps) => {
  const { copy, navigationLinks, textContentOnly, goBackButtonText, video } =
    props;
  ...
  useEffect(() => {
    if (currentSection === "" && !textContentOnly) {
      updateCurrentSection(navigationLinks[0].slug ?? null);
    }
  }, [currentSection, navigationLinks, updateCurrentSection, textContentOnly]);
  ...
  return (
    <DynamicNavigationLayout
      anchorNavigation={
        textContentOnly ? "" : (navigationLinks[0].slug ?? null)
      }
    >
      <div
        className="relative"
        id={textContentOnly ? "" : (navigationLinks[0].slug ?? undefined)}
        ref={startPageRef}
      >
        <DesktopVersion
          {...props}
