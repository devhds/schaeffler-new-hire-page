...
type TeaserItem = TeaserTypes["teaserItems"][number];

interface TeaserItemProps extends TeaserItem {
  index: number;
  itemsLength: number;
}
...
const TeaserItem: React.FC<TeaserItemProps> = ({
  ...
  const href = externalPageLink ? externalPageLink : internalPageLink;
    ...
    <>
      ...
      >
        <motion.div
          ...
          initial="default"
          animate={
            currentHoveredElementId === id && isHovered ? "hidden" : "default"
          }
          className=""
        >
          <LabelText
        ...
        <motion.div
          ...
          initial="default"
          animate={
            currentHoveredElementId === id && isHovered ? "active" : "default"
          }
        >
          <Headlines element="h4" text={text} color="text-primary-white" />
          <motion.div
            className="mt-2"
            variants={{
              disable: {
            ...
            initial="disable"
            animate={
              currentHoveredElementId === id && isHovered ? "active" : "disable"
            }
          >
            {description && (
              <BodyText
...
interface TeaserWithImageProps extends TeaserItem {
  animate: boolean;
  index: number;
}
...
const TeaserWithVideo: React.FC<TeaserItem> = ({ video }) => {
  return (
    <>
      <video
        src={video + "#t=0.001"}
