...
import { ImageFullScreenTypes } from '@/payload-types'

export interface ImageFullScreenProps extends ImageFullScreenTypes {
  classNames?: string
}
...
const ImageFullScreen: React.FC<ImageFullScreenProps> = ({
  ...
  const textOffset = useTransform(scrollYProgress, [0.35, 0.55], ['-16rem', '-20rem'])
  ...
  return (
    ...
    >
      ...
      >
        {typeof image === 'object' && image?.mimeType?.includes('image') && image.url && (
          <Image className="h-screen w-full object-cover" src={image.url} alt={image.alt} fill />
        )}
        <motion.div
          style={{
      ...
      >
        <Headlines element="h1" text={copy.headline} color="text-primary-white" />
        <LabelText
          className="pt-6"
